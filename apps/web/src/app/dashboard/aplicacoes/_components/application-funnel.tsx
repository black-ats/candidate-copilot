  ...
  return (
    <Card className="p-3 sm:p-4">
        ...
        {funnelStages.map((stage) => {
          ...
          return (
            <div key={stage.key} className="flex items-center gap-2 sm:gap-3">
              <div className="w-16 sm:w-20 text-xs text-navy/60 text-right flex-shrink-0 truncate">
                {stage.label}
              </div>
      ...
      {(data.aceito > 0 || data.rejeitado > 0 || data.desistencia > 0) && (
        <div className="mt-4 pt-3 border-t border-stone/20 flex flex-wrap items-center gap-2 sm:gap-4 text-xs text-navy/50">
          {data.aceito > 0 && (
